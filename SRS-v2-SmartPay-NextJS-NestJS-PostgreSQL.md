...
| **Amendments** | 2.1 — added NFR-12 and §8.1, responsive layout. 2.2 — added §2.7 as-built deviations, §8.2 list-screen conventions (NFR-13) and §8.3 interface system (NFR-14). 2.3 — ORM changed from Prisma to TypeORM; added §2.8 persistence layer. 2.4 — identity updated to the navy/sky logo palette; all data-shape names follow the database columns; all primary keys sequential. 2.5 — Module 3 built; deviations 8–10 added. 2.6 — CNIC front/back on customers and guarantors; `files` keyed by integer with owner columns and no FK (deviations 11–12). 2.7 — Module 4 built; the plan preview is priced server-side and pickers read unpaged lookups (deviations 13–14). 2.8 — one purchase price replaces the cost/sale pair, settling amendment questions 6 and 9 (deviation 15); the markup rupee override leaves the form (deviation 16). 2.9 — Module 5 built: the agreement prints white-on-navy from its own route group, and the 16 clauses ship as replaceable defaults (deviations 17–18). 2.11 — Modules 6 and 7 built; the ledger downloads as a data-built PDF and print no longer inherits dark mode (deviations 19–20). 2.12 — Module 12 built on a typed settings registry; the punctuality bands and loyalty tiers became configurable (deviations 21–22). 2.13 — Module 1 built; BR-09 profit maturity moved into the formula package. 2.14 — Module 9 built; the role split verified end to end against a real operator token (deviation 23). 2.15 — Module 11 built: the viewer computes the before/after diff server-side, and FR-AUD-03 is enforced by the controller having no write route at all. 2.16 — Module 10 built on a per-entity registry; staff accounts join the bin (deviations 24–25). 2.17 — Module 8's core built: BR-08/BR-10/BR-11 in the formula package, capital and expenses as records (deviations 26–27). 2.18 — Module 8's second pass: Top Performer, the client profile modal and export (deviations 28–29). 2.19 — Module 7 completed: the recovery register and immutable archive (deviation 30). 2.20 — Module 13's foundation: BR-21/22/24/24a and BR-15/17 in the formula package, investor CRUD, deposits, withdrawals and adjustments (deviation 31). 2.21 — Module 13's screens: investor register and detail with the derived KPI strip and the three hand-entered movements. 2.22 — investors are linked to contracts: BR-18/19 recovery splitting, funding at activation with FR-CON-13's guards, and investor balances reading real deployments (deviation 32). 2.23 — the funding panel reaches the screen: the contract form allocates capital on create, the ledger shows who funded the deal, and `house_funded_amount` is read from the funding rows rather than assumed (deviations 33–34). 2.24 — BR-20 loss allocation: cancelling with a write-off and purging a funded contract both settle the funders' unrecovered capital, and a written-off contract stops tying that capital up (deviations 35–38). 2.25 — BR-25 replaces BR-10: the Summary Report's net balance nets investor participation out of outstanding and unmatured profit, and FR-SUM-11's investor position block says what was netted out (deviations 39–40). 2.26 — the agreement downloads as a data-built PDF, like the recovery ledger, and the installment schedule carries what was received against each row (deviations 41–42). 2.27 — every contract must name an investor: house-funded activation is refused by the create DTO and by the form, and the fundable-investor lookup opens to operators so they can still write contracts (deviations 43–44). 2.28 — house funding is gone entirely: investors must cover the purchase price exactly, `house_funded_amount` and BR-25's house figures are removed, and the net balance is own capital less expenses (deviations 45–46). 2.29 — the Funding Register is added as FR-IVT-16: `GET /reports/funding` and a screen of its own listing every funded contract with its investors, classified sole or joint, rolled up per investor and filterable. 2.30 — BR-26 fix: recovery slices and profit entitlements are weighted by the amounts invested rather than the rounded `share_pct`, which was losing rupees on a fully-collected joint deal (deviation 47). Cycles, statements and the flow report remain. Simulation mode (FR-SUM-04-v2) and the Client Search modal remain. 2.10 — Module 14 (Customer Messaging / WhatsApp) added in two stages, with BR-27 and `message_log`. |
...
Modules 1 to 7 and 9 to 12 are built; Module 8 lacks only simulation mode. These decisions differ from the text above; each was
...
| 26 | §2.3 — an operator may read the summary report | Module 8 is **admin only** | the deal rows alone would be fine for an operator, but capital, expenses and net balance are the business's own position (NFR-15) and sit on the same screen. Splitting the response by role would mean two shapes for one page; a separate read-only route is the cleaner answer if an operator view is ever wanted |
| 27 | FR-SUM-01-v2 / FR-SUM-09 — server-side pagination | the page is cut **after** every deal is summarised, not in SQL | the portfolio counters, the deal-counter shares and the missing-data lists are whole-portfolio figures. Deriving them from a page would make them change as you turn it — which is the class of fault §9.6 records. Cost is three queries regardless of size; if the portfolio ever outgrows that, the totals move to their own aggregate rather than the page growing |
| 28 | FR-SUM-07 — Top Performer ranks clients | a client's score is the **value-weighted** mean of their deals' BR-11 scores | BR-11 is defined per deal and the SRS does not say how to combine them. A flat average would let a small plan paid perfectly cancel a large one running late, which is the opposite of what the ranking is for: it is meant to say who is worth writing more business with. With one deal — the common case — the weighting is a no-op |
| 29 | FR-SUM-08 — export as print, image and clipboard | **PDF instead of image**, plus print and clipboard | the same reasoning as deviation 19: a DOM capture is a blurry raster that cannot be searched and inherits the viewer's appearance. Landscape A4, because fourteen columns do not fit portrait at a readable size. The clipboard copy is **tab-separated**, which a spreadsheet pastes straight into cells. All three act on the rows **currently shown**, filters and sort included — exporting the whole portfolio from a screen deliberately narrowed to one client would be the wrong answer, and the sheet states how many rows of how many it holds |
| 30 | FR-REC-08 — snapshots listed per contract | an archive is also readable at `GET /recovery/snapshots/{id}`, and there is **no update or delete route anywhere** | an archived copy outlives the state it was taken from, so addressing it only through its contract would imply otherwise. Immutability is enforced the same way FR-AUD-03 is — by the absence of a write route, not a guard that could be misconfigured. The payload stores the *rendered* ledger, not a reference to it, so it still says what it said after the next payment lands |
| 31 | FR-IVT-05/06 — a deposit or withdrawal carries a `note` | there is **no note field**; `reference` carries it | §5.17 of the amendment lists the columns and has no `note`. Rather than add a migration for a field the schema deliberately omits, `reference` is documented as the place for a cheque number, a bank reference or a short remark. `reason` remains separate and required on an Adjustment |
| 32 | FR-BIN-03 / BR-20 — purging a funded contract writes Loss rows | purging a funded contract is **refused** until loss allocation is built | the funding rows carry someone else's stake. Deleting them without writing the offsetting Loss would leave that investor's balance quietly wrong — worse than refusing. The blocker names the rule and says plainly that it is not built, so the refusal is not mistaken for a defect |
| 33 | FR-CON-11 — the panel proposes a pro-rata allocation across all active investors, and shows each line's principal/profit split before it is written | the panel starts **empty**, and each line is added deliberately; the bucket split is shown *after* activation, on the ledger's Funded by card | an auto-proposal spreads capital across every investor by default, which is a decision about someone else's money made by a checkbox. Adding each funder deliberately is the safer default for a business where a deal is usually backed by one or two people known to the owner. The setting `auto_allocate_funding` is therefore not implemented. The bucket split is BR-22's answer at the moment of writing, and the server decides it — showing a prediction beforehand would be a second implementation of the same rule, free to disagree with the first |
| 34 | — | `house_funded_amount` is computed from the stored funding rows on every detail read, not stored on the contract | the contract has no column for it on purpose. A stored copy would be a second source of truth for BR-14 and could drift from the funding rows; deriving it means the two cannot disagree. Before funding existed the mapper defaulted it to the full cost price, which was correct then and became a silent lie the moment a contract could be funded — every detail path now passes the real figure |
| 35 | BR-20 — where `loss_participation` is false, an **Adjustment** crediting the unrecovered amount is written *instead* of a Loss | **both** are written: the Loss that happened, and an Adjustment of the same amount crediting it back | the Adjustment alone assumes the deployment keeps subtracting from the investor's balance forever. It does not — a settled contract stops counting as deployed, or the investor's capital would show as tied up in a dead contract for good. A lone credit would then *add* to their balance and pay them for a loss they did not suffer. The pair nets to zero, leaves the investor whole exactly as BR-20 intends, and puts on the ledger what actually happened rather than only its remedy |
| 36 | FR-CON-16 — soft-deleting a funded contract requires loss settlement in the same transaction | soft delete settles **nothing**; only a write-off cancellation and a purge do, which is what BR-20 itself names | a soft delete is reversible by design — the record goes to the Recycle Bin and FR-BIN-02 restores it. Loss rows are append-only and cannot be unwritten, so settling on soft delete would make a reversible act irreversible in the investor ledger. The capital stays counted as deployed while the contract sits in the bin, so nothing is understated in the meantime |
| 37 | — | a Loss row written during a **purge** carries no `contract_id`; the contract number is in its reason instead | `investor_transactions.contract_id` is ON DELETE RESTRICT, so a Loss pointing at the contract would block the very purge it was written for. Rows written at cancellation keep the link and lose it only if that contract is later purged. A foreign key to a row that no longer exists tells a reader nothing; the number in the reason survives, and so does the audit entry |
| 38 | — | BR-20 runs **once** per contract: a contract already cancelled with `write_off` is skipped when purged | reaching the allocation twice would write a second set of Loss rows and charge every funder again for one failure. The guard reads the same settled flag that stops the capital counting as deployed, so the two cannot disagree about whether a contract is finished with |
| 39 | BR-25 — `houseUnmaturedProfit` includes `Σ unmaturedRetailMargin_c` | the retail-margin term is **absent**, because it is structurally zero | deviation 15 collapsed the cost/sale pair into one purchase price, so a contract has no margin between what was paid for the unit and what it was written at. The whole of the profit is the markup, which the first term already covers. Adding a term that can only ever be zero would suggest a second source of profit this business does not have |
| 40 | BR-25 — house figures replace BR-10's | both are reported: `total_outstanding` and `unmatured_profit` stay as the portfolio's figures, with `house_outstanding` and `house_unmatured_profit` beside them, and `net_balance` computed from the house pair | the customer still owes the whole outstanding balance and FR-SUM-03 needs to say so — replacing that figure would make the register disagree with the ledgers underneath it. What BR-25 is actually replacing is the *net balance*, and that is computed from the house figures alone. The house block is hidden entirely when no investor holds any part of the portfolio, where the two would be identical |
| 41 | FR-INV-06 — the agreement prints through the browser, with a PDF endpoint in Phase 2 | it downloads as a **client-built PDF** (`invoice-pdf.ts`), drawn from the invoice data with jsPDF exactly as the recovery ledger is; the browser print stays alongside it | at the counter the operator wants the file, not a print dialog to configure. Building from data rather than capturing the page gives selectable vector text in a ~39KB file that looks the same on every machine, where a DOM capture is a multi-megabyte raster that inherits the viewer's appearance — the fault that once printed a ledger white on white. No server endpoint was needed for it. Print is kept as the honest what-I-see copy and because only the browser can choose a printer |
| 42 | FR-INV-03 — the schedule's Received column is signed by hand | it prints the amount **applied to that installment** and the date it cleared, in two columns, and stays blank only where no money has been applied | the same document is used twice: as an agreement to sign, and as the statement a customer asks for months later. Blank cells still print for an unpaid row, so the hand-signed use is untouched. The figures come from the same FIFO allocation the recovery ledger runs (BR-13), so the printed copy and the ledger screen cannot disagree — which is also why the date is the day the *installment cleared*, not the day a payment arrived: one payment can settle several rows |
| 43 | FR-CON-13 — *"A contract may be activated fully house-funded"* | a create with no funding line is **refused**, by `CreateContractDto` (`fundings` is required, `ArrayMinSize(1)`) and by the form before it submits | the business does not put its own capital into a deal — every contract is backed by an investor, so an unfunded one is a mistake rather than a variant. It is refused in the DTO as well as the form because a rule that lives only in a browser is not a rule. What is refused is a contract with *no* investor; the remainder above the funded amount is still house money, exactly as BR-14 says, so partial funding is unaffected. `UpdateContractDto` is unaffected: `PartialType` makes every field optional again, and FR-CON-15 already refuses funding on a PATCH |
| 44 | NFR-15 — investor data is admin-only | `GET /investors/fundable` is open to **operators**; the rest of the investor module stays admin-only | deviation 43 makes a funder compulsory, and operators write contracts — refusing them the list would refuse them their job. The opening is the narrowest that works: active investors, their name and what they have free. `/investors`, `/investors/:id`, every movement route and the Summary Report all still return 403 for an operator, verified against a real operator token |
| 45 | FR-CON-13 / BR-14 — a contract may be part-funded, the house covering the remainder | investor funding must equal `cost_price` **exactly**; `house_funded_amount` is removed from every response and the form blocks a save until the allocation reaches the cost | the business buys nothing with its own capital. A deal short of its cost is not part-funded, it is one still waiting for another investor, so the panel and the API both say how much is left to find rather than quietly absorbing it. BR-14's `houseFunded` and the mapper's fallback are deleted rather than left returning zero, so no screen can imply a house stake that cannot exist |
| 46 | BR-25 / FR-SUM-11 — the Summary Report nets investor participation out of house figures | `house_outstanding` and `house_unmatured_profit` are **removed**; `net_balance` is own capital less expenses, and the report's second card is the investor position | with every deal bought outright by investors, the house share of each is zero by construction — the two figures could only ever report zero, and a net balance netting them is arithmetic on nothing. What remains true is that the customer owes the whole outstanding balance and the whole markup will mature; both are simply owed onward. **Consequence, verified:** profit follows the capital share, so a 100% funder takes 100% of the markup and the business earns nothing from a contract. A house cut of the markup would need to be added deliberately |
| 47 | BR-17 / BR-18 / BR-26 — a funder's slice and entitlement follow their `share_pct` | both are weighted by the **amount invested against the cost price**; `share_pct` is display only and no longer appears in any money arithmetic | `share_pct` is `DECIMAL(5,2)`, so a 90,000 stake in a 140,000 unit stores as 64.29% when it is 64.285714…%. Splitting by that rounded figure lost 6 rupees on a contract collected to the last rupee: 48,994 of a 49,000 markup matured and the remaining 6 was credited to a house with no share of the deal. Weighting by the amounts is exact, and `allocate` still places BR-26's residual on the largest share, so the parts sum to the whole. Nothing is stored, so the correction applies to every existing contract on the next read |
